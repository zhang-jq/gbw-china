package com.spider.gbwchina.downloader;
...
public class HttpClientGenerator2 {
    private transient Logger logger = LoggerFactory.getLogger(this.getClass());
    private PoolingHttpClientConnectionManager connectionManager;

    public HttpClientGenerator2() {
        Registry reg = RegistryBuilder.create().register("http", PlainConnectionSocketFactory.INSTANCE).register("https", this.buildSSLConnectionSocketFactory()).build();
    ...
    public HttpClientGenerator2 setPoolSize(int poolSize) {
    ...
    private void generateCookie(HttpClientBuilder httpClientBuilder, Site site) {
        if(site.isDisableCookieManagement()) {
            ...
            while(var4.hasNext()) {
                domainEntry = (Entry)var4.next();
                Iterator cookie2 = ((Map)domainEntry.getValue()).entrySet().iterator();

                while(cookie2.hasNext()) {
                    Entry cookieEntry = (Entry)cookie2.next();
                    BasicClientCookie cookie1 = new BasicClientCookie((String)cookieEntry.getKey(), (String)cookieEntry.getValue());
                    cookie1.setDomain((String)domainEntry.getKey());
